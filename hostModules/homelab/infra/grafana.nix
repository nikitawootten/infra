...
let
  ...
  idp =
    let
      domain = config.homelab.infra.kanidm.domain;
    in
    {
      ...
      oidc_discovery = "https://${domain}/oauth2/openid/${clientId}/.well-known/openid-configuration";
      rfc8144_authorization_server_metadata = "https://${domain}/oauth2/openid/${clientId}/.well-known/oauth-authorization-server";
      ...
      token_signing_public_key = "https://${domain}/oauth2/openid/${clientId}/public_key.jwk";
    };
in
{
  ...
  config = lib.mkIf cfg.enable {
    services.grafana = {
      ...
      settings = {
        ...
        "auth.generic_oauth" = {
          ...
          use_pkce = true;
          role_attribute_path = "contains(roles[*], '${adminGroup}') && 'Admin' || contains(roles[*], '${editorGroup}') && 'Editor' || 'Viewer'";
        };
      };
      provision = {
        enable = true;
    ...
    services.kanidm.provision.groups = lib.genAttrs groups (_: {
    ...
    services.kanidm.provision.systems.oauth2.${clientId} = {
      ...
      scopeMaps = lib.genAttrs groups (group: [
        "email"
        "openid"
        "profile"
        "roles"
