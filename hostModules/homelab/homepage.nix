...
let
  ...
  enableDefaultTrueOption = description: lib.mkOption {
    type = lib.types.bool;
    default = true;
    inherit description;
  };
  ...
  toHomepageConfig = itemsConfig: let
    keys = builtins.attrNames itemsConfig;
    items = builtins.map (name: { inherit name; value = builtins.getAttr name itemsConfig; }) keys;
    sorted = sortByPriority items;
    filtered = builtins.filter (item: item.value.enable) sorted;
  in
    builtins.map (item: {
      "${item.name}" = item.value.config;
    }) filtered;

  toParentHomepageConfig = itemsConfig: let
    transformed = builtins.mapAttrs (name: item: item // { config = toHomepageConfig item.config; }) itemsConfig;
  in
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    services.homepage-dashboard = let
      service-cfg = config.services.homepage-dashboard;
    in {
      enable = true;
