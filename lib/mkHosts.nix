{ nixpkgs, ... }:
{ hosts, homeConfigs ? null, configBasePath, defaultModules ? [ ]
, specialArgs ? { }, }:
let
  ...
  mkSystem = hostname:
    ...
    let
      homeModules = if (homeConfigs != null
        && builtins.hasAttr "${username}@${hostname}" homeConfigs) then
    ...
    in nixpkgs.lib.nixosSystem {
      ...
      modules = [
        ({ lib, ... }: {
          ...
          imports = [ "${configBasePath}/${hostname}" ];
        })
