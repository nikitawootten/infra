...
{ homes, configBasePath, defaultModules ? [ ], specialArgs ? { }
, stateVersion ? "23.11", }:
let
  parseEntry = entry:
    let
      ...
      hostname = if builtins.length entryItems == 3 then
        builtins.elemAt entryItems 2
      else
        null;
  ...
  mkHomeManagerCommon = imports: hostname:
    ({ lib, username, system, ... }:
      let
        homeFolderPath = if builtins.isNull hostname then
          "${configBasePath}/${username}"
        else
          "${configBasePath}/${username}@${hostname}";
        homeFilePath = "${homeFolderPath}.nix";
        homeImports = (lib.lists.optional (builtins.pathExists homeFolderPath)
          homeFolderPath)
          ++ (lib.lists.optional (builtins.pathExists homeFilePath)
            homeFilePath);
      in {
  ...
  mkHomeManagerNixOsModules = entry:
    { system, modules ? [ ], }:
    let
      parsedEntry = parseEntry entry;
      username = parsedEntry.username;
